...
export default function PostCard({post}: { post: any }) {
    ...
    return (
        <Card w={isLargerThan1200 ? "auto" : "75%"} borderRadius={"20px"} boxShadow={shadow} cursor={"pointer"} transition={"box-shadow 0.2s ease-in-out"} _hover={{ boxShadow: shadowHover }}>
            <Link href={"/posts/" + post.type + "/" + post.id}>
                <CardHeader>
                    <Flex gap='0'>
                        <Flex flex='1' gap='4' alignItems='center' flexWrap='wrap'>
                            <Avatar size={isLargerThan1200 ? "md" : "sm"} name={postUser.firstName + " " + postUser.lastName} src={postUser.image} flexShrink={1} />
                            <Box>
                                <Heading size={isLargerThan1200 ? "sm" : "xs"}>{postUser.firstName + " " + postUser.lastName}</Heading>
                                <Text fontSize={isLargerThan1200 ? "sm" : "xs"} color='gray.500'>{postUser.city.name}</Text>
                            </Box>
                        </Flex>
                    </Flex>
                </CardHeader>
